...
(function () {
  ...
  function causaProvavel(mensagem) {
    ...
    if (/entregas diferentes/i.test(texto)) {
      return 'Algum arquivo colado no Apps Script ficou de uma versão anterior. A mensagem acima diz qual: cole a versão atual dele e publique uma nova versão.';
    }
    if (/Falta o arquivo Versoes\.gs/i.test(texto)) {
      return 'O arquivo Versoes.gs não está no Apps Script. Ele faz parte da entrega e confere se os demais arquivos são da mesma versão.';
    }
